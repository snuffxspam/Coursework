...
void Univer::changeStudent(string zach_num, int field) {
		...
		Student* t = getByZach(zach_num); // записать указатель на изменяемого студента
		...
		sne: 
			...
			// Проверяет, если в tmp введён 0, чтобы дать пользователю возможность
			// прекратить редактирование поля

			// проверка фамилии на спец символы
			if (!checkSpec(tmp, 0)) {
				cout << CHR_ERR << endl; // CHR_ERR - define из языкового файла
				goto sne;
			}
			t->_sname = tmp; break;
		} 
		...
		jdate_input: 
			...
			// проверка того, что год является числом и попадает в указанный диапазон
		...
		semnum:
			...
			// проверка значения на попадание в заданный диапазон (семестров всего 12)
...
/*
	...
	Вариант 93. 
	Найти и распечатать все данные о 
	студентах, которые успевают с наибольшим 
	и наименьшим успехом, с поиском среди лиц 
	с указанием интервала года рождени

*/
void Univer::var93query(int se, int ee) {
	// массив cредних арифметических оценок студентов
	float* avgArr = new float[N];
	// массив индексов студентов в списке
	int* inds = new int[N];

	// заполняем массивы
	for (int i = 0; i < N;i++) {
		Student* tmpS = &((*_students)[i]);
		if (tmpS->_bdate.year <= ee && tmpS->_bdate.year >= se) {
			avgArr[i] = tmpS->fullAvg();
		}
		else {
			avgArr[i] = -1; // студент не подходит по запросу
		}

		inds[i] = i; // записываем индекс в массив индексов
		// используется в дальнейшем при сортировке студентов
	}



	// сортировка пузырьком массива средних арифметических
	// при этом параллельно с элементами массива
	// двигаем и индексы студентов во втором массиве, чтобы
	// знать, у какого студента такое среднее значение
	float t;
	int ti; // переменные для хранения временных значений
	...
	cout << endl;
	cout << "Наименее успевающий студент: " << endl;
	for (int i = 0; i < N;i++) {
		if (avgArr[i] == -1)
			continue;

		(*_students)[inds[i]].fullIntroduce();
		(*_students)[inds[i]]._zach->showAll();
		cout << endl;
		break;
	}

	cout << "Наиболее успевающий студент: " << endl;
	for (int i = N - 1; i >= 0;i--) {
		if (avgArr[i] == -1)
			continue;
