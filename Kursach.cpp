...
// объявить методы до главной функции, чтобы дать понять компилятору
// что они будут заданы позже
void input_data();
void menu();
 
// Указатель на класс университета, который будет использован позже
Univer *u;
...
int main()
{
	...
	// Создать университет МИРЭА и записываем его адрес в указатель
	u = new Univer("MIREA");

	// Зациклить показ меню
	while (1)
		menu();
...
// Функция меню, при помощи которой вводится действие, совершаемое с данными
void menu() {
...
menu_n: 
	...
	cout << "5. Показать выборку (вариант 16)" << endl;
	cout << "0. Выход" << endl;
	cout << "> ";

	// Ввод номера пункта меню в формате строки (да, строка, а не число
	// потому что так удобнее проверять на правильность ввода)
	getline(cin, n);
	
	// проверка введённых данных - а именно
	// * данные являются числом и * это число входит в диапазон пунктов меню
	if (!isValidInt(n) || (stoi(n) > 5 || stoi(n) < -1)) {
		// если что-то не так - очистка консоли и возврат на метку выше
		cls;
		goto menu_n;
	}
	...
	case 5: {
		...
		string y1, y2;
	entyr1:
		cout << "Введите год (левая граница) > ";
		getline(cin, y1);
		if (!isValidInt(y1)) {
			cls;
			cout << INVINP << endl;
	...
	entyr2:
		cout << "Введите год (правая граница) > ";
		getline(cin, y2);
		...
		u->var93query(stoi(y1), stoi(y2)); // передать в функцию выборки
		// левую и правую границу года рождения
